...
`;
  ...
  const [initalState, SetinitalState] = useSetState<MessageListStates>({initalState: {a:123}});
  ...
  useEffect(() => {
    console.log('rightDragRef', rightDragRef)
    rightDragRef.current.onmousedown = (e) => {
      let _e = e;
      ...
      document.onmousemove = (_event) => {
        _e = _event;
        // console.log('2222',leftDomRef, width, `${contentRef.current.offsetWidth - rightDomRef.current.offsetWidth + (_e.clientX - firstX)}px`)
        leftDomRef.current.style.width =  `${contentRef.current.offsetWidth - width + (_e.clientX - firstX)}px`
        // leftDomRef.current.style.width = '200px';
        // 可扩展上下拖动等
        // switch (dir) {
        //   case 'horizontal':
        //     leftDomRef.current.style.width = `${contentRef.current.offsetWidth - rightDomRef.current.offsetWidth + (_e.clientX - firstX)}px`;
  ...
  return <div className={styles.InspectionLead} >
    ...
    >
      <div className={'condition'}  ref={contentRef}>
        ...
        <Provider value={{initalState:initalState.initalState, SetinitalState}}>
